...
<%@page import="com.tcs.ilp.lms.bean.MagazineBean1"%>
<%
	Object magBean = request.getAttribute("viewMagazineList");
	ArrayList<MagazineBean1> magList = (ArrayList<MagazineBean1>) magBean;
	System.out.println("Size: " + magList.size());
%>
...
<ul>  
		...
		<div id="content" style="float: left; width:60%; background-color:black; height: 850px; color:white;">	
 		...
 		<br></br>
 		    ...
 		    <center>
			<h5>Sorry!!No Magazines issued by this member</h5>
			</center>
 ...
 <%} else { %>
		...
		<form action="ReturnServlet?req=magstatuschange" method="post" onsubmit="return check2()">
	...
	<div class="a" id="div1" align="center" >
		...
		<h3 id="cdlist">Issued Magazines List</h3>
		<br>
...
<%
		for (int i = 0; i < magList.size(); i++) {
			MagazineBean1 e = magList.get(i);
	%>
	<tr>
		<td><input type="checkbox" value="<%=Integer.toString(e.getMagazineId())%>" name="radio1"></td>

		<td><%=e.getMagazineId()%></td>
		<td><%=e.getMagazineName()%></td>
		...
		<br></br>
		<input type="submit" value="Return Magazine" name="returnmag"></input></form>
<%} %>
    </div>
	</div>
